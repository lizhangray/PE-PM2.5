...
def get_BLH_Dataset(data_root=""):
    ...
    df_test_Xys['file_Id'] = df_test_Xys['file_Id'].map(lambda x: os.path.join(img_prefix, x))  # warning
...
def get_PM_Dataset(data_root=""):
    """该数据集的最大最小值会用于训练和验证阶段！！！"""
    print("Chenk dataroot: ---", data_root)
    if "BLH" in data_root or "Daytime" in data_root:
